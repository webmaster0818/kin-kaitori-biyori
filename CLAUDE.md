...
### 2026-08-27 GSC再測定 → 🚨サイト全体の金価格が4か月前のまま（MediaXAI「最新のGSCデータ見た上で、ネクストアクション実行して」）✅本番反映済み
- **GSC**: 8月(1-26) click10 / imp2,530 / pos30.0（7月 13/2,096/32.0）。表示は伸びたがCTRは0.62%→0.40%に低下。
- 🚨 **最大の発見＝表示ではなく事実の誤り**。**「2026年4月現在、K24は1gあたり約15,200円」という4か月前の固定価格が、TOPページ・/faq/・記事27本に残っていた**。実勢は**24,808円**＝**39%の過小表示**。プラチナは5,300円→実勢10,142円（**48%過小**）、銀は130〜145円→実勢383円（**65%過小**）、メイプルリーフ1oz 47万円→実勢約77万円。**金買取サイトが自分のトップページで金の値段を4割安く書いていた。**
  - **なぜ残ったか**: 2026-06-12に「サイト全体で誤り固定価格ゼロ」と記録したが、あの一括修正の対象は**218の地域ページと11の純度/品目ページだけ**。TOP・FAQ・一般記事（売り時／税金／確定申告／純度の見分け方／重さの量り方／プラチナ／銀／遺品整理／ブランド4本…）は**一度も監査していなかった**。CLAUDE.mdに4回「固定価格残存は毎回grep監査」と書きながら、**監査範囲を`app/articles/{地域,純度}`に限定していたのが盲点**。
- **対応（2波・計30ファイル）**: すべて `gold-spot-prices.json` 由来のライブ式に置換（毎朝11:05の日次cronで自動更新される＝二度と陳腐化しない）。
  - 各ファイルに `/* live-price-consts */` ブロック（`LP_K24`/`LP_K18`/`LP_K14`/`LP_K10`/`LP_PT`/`LP_PT900`/`LP_PT850`/`LP_AG`/`LP_AG925`/`LP_OZ`/`LP_DATE`）を注入。スクリプト＝`/tmp/fix_stale_prices.py`・`/tmp/fix_stale2.py`（冪等）。
  - 派生額（10g・1kg・1oz・計算例・業者比較例・金歯の重量帯）も**単価から計算する式**に変更。
  - `kin-uridoki-2026` の**予測レンジ表は4月時点の強気シナリオ(15,500〜17,000円)を実勢が追い越していた**ため、現在値からの機械的なレンジ（+10〜+30% / −5〜+10% / −25〜−10%）に置換。
  - **歴史推移の行（2016〜2024年・2022年4月など過去の事実）は対象外**として温存。
  - ブランド固有の買取レンジ（カルティエ/ブルガリ/ティファニー/ヴァンクリーフの「約15万〜25万円」等）は地金から算出できないため数値は据え置き、「2026年4月**現在**」→「2026年4月**時点の調査では**」とas-of表記に正した。**⚠️これらは再調査が別途必要（未処理）**。
  - ⚠️ **TDZ罠**: 定数ブロックを `ArticleSchema()` より後ろに置くと `used before its declaration`、`_LPT` を `_LPP` より後ろに置いても同じ。**必ず import 直後にまとめる**。
- **カニバリ解消（GSC実測）**: 「金 買取 必要なもの/書類/本人確認/身分証」等**6クエリが hajimete(pos27-40) と tetsuzuki-guide(pos58-77) の両方で表示**＝計184imp・0クリック。8/5に分離を宣言したが**本文は分離しきれておらず「持ち物リスト」「本人確認」の重複セクションが丸ごと残っていた**。
  - `hajimete` = **「金買取に必要なもの」の受け皿**に確定（h1・schema headlineをmetadata titleへ整合＋冒頭に結論box「必須は本人確認書類1点」＋4月の誤価格表をライブ表に）。
  - `tetsuzuki-guide` = **「未成年・年齢」の受け皿**に確定（pos21.3＝このページ唯一の勝ち意図）。重複2セクションを要約＋hajimeteへの送りに圧縮し、「何歳から売れる？」（民法上18歳／店の運用は20歳以上・高校生不可もある）＋同意書の記載事項＋年齢別テーブルを新設。h1/schema/パンくず/FAQも未成年先行に。
- **喜平の即答化**: 「喜平ブレスレット 10g/30g 買取価格」が**pos11.5/10.6で0クリック**（**駅名以外で唯一の1ページ目隣接クエリ**）。titleが【2026年7月】のうえ数値が無かった → `金ブレスレット買取相場｜喜平10gは約186,060円・30gは約558,180円【8月27日更新】`（日次自動更新）。title/h1/schemaの三者を個別に確認済み。
- 検証: out/ と本番の両方で陳腐化リテラル**0件**・実値反映を12ページでcurl確認・**Indexing 36/36**・sitemap再送信OK。⚠️ CFエッジは**POPごとに反映がずれる**（1回目のcurlで旧値、数十秒後に新値）。1回の不一致でバグと判断しない。
- ⚠️ **地域/駅名クエリは今回も見送り**: 駅名12クエリが**pos7〜13で合計375表示・0クリック**、横浜は154表示/pos47.9で0クリック。**gakkiの渋谷・新宿と同じ形が2サイト目**。8/24に作った駅ページ2本は公開3日でまだ着地が名古屋ページのまま＝**判定はまだ出ていない**（2〜3週後に再測定）。それまで地域レイヤーに追加投資しない。
- **次に見るもの（2〜3週後）**: ①喜平10g/30gクエリ（pos10.6/11.5・28imp）がクリックに変わるか ②「金 買取 必要なもの」系184impの着地が hajimete に一本化されるか ③駅名クエリの着地が駅ページに移るか。

### 2026-08-29 ①②③チェック→薄い5ページの統合301（MediaXAI「①②③チェックの上、ネクストアクション実行して」）✅本番反映済み
- 🚫 **①②③はまだ測定できない**。GSCが返す最新日は **2026-08-26** で、施策のデプロイは 8/27。**施策後のデータが1日も入っていない**（GSCは2〜3日遅れる／そもそも私は「2〜3週後に測る」と書いた）。**ベースラインだけ確定させた**:
  ```
  ① 喜平ブレスレット系      37imp / 0click（30g pos10.6・10g pos11.5）
  ② 必要なもの・書類系     185imp / 0click（hajimete pos27〜40／tetsuzuki pos58〜72で二重表示）
  ③ 駅名クエリ           402imp / 0click（本陣7.9・栄生8.4・久屋大通9.3・ささしま8.7）
  ```
  次に見るのは **9月中旬**。今日の数字と比べる。
- **オンページの機会はほぼ枯れている**（実測）: 駅名を除き **pos≤25 × imp≥8 のクエリは4行・計55imp・0click** しかなく、しかも全て8/27に着手済みのもの。**新しく拾える上位クエリが無い**ことを確認した。
- **代わりに実行＝日次cronのリントが出し続けていた警告の解消**。`scaled-content-lint`（daily-gold-update.sh に組込済み）が **THIN=5/289** を毎日出していた:
  ```
  kin-yusou-anzen(91) / kin-kaitori-tesuryo(92) / kin-kaitori-soubadori(95)
  kin-kaitori-sokujitsu(96) / kin-kakaku-suii(97)     ※独自量の中央値は143
  ```
  5本とも**「ポイント／FAQ／Q&A／関連記事／{テーマ}の買取で使える主要サービス比較／まとめ」の同一テンプレ**で、固有部分はポイントとFAQのみ。見出しに **「相場通りに売るにはの買取で使える主要サービス比較」** という生成崩れも残っていた＝**Scaled Contentの典型**（takushoku被弾と同型）。
- **90日実績を確認してから統合**（失うものが無いことを先に確かめる）: 5本とも **0クリック**、内部リンク **1〜2本**（実質孤立）。
  ```
  kin-kaitori-tesuryo   15imp pos39.7 → kin-kaitori-tesuryo-hikaku（13社比較 pos9.9）
  kin-kaitori-soubadori  3imp pos85.0 → kin-takaku-uru（高く売る＝手取りを相場に近づける）
  kin-kakaku-suii        0imp         → /souba-dashboard/（推移チャートの本体 pos3.9）
  kin-kaitori-sokujitsu  0imp         → kin-kaitori-tetsuzuki-guide（当日の流れ・所要時間）
  kin-yusou-anzen       45imp pos69.8 → kaitori-houhou-hikaku（宅配を含む買取方法の比較）
  ```
- 実装: `public/_redirects` +10行（スラッシュ有無の両方）／内部リンク6ファイル差替／`articles-metadata.json` 287→282／ページ削除。
