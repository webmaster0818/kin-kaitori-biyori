...
### 2026-08-02 D2(名古屋モデルケース)+残P1(上田)（MediaXAI「D2進めて」→催促を受けて完了）✅本番反映済み
⚠️**着手宣言(7/31)後に他サイト作業が立て込み、実装が止まっていた**。8/2にMediaXAIから「終わった？」と催促を受け、正直に未完了と報告して即完了させた。**着手宣言したら完了報告までがタスク**（[[long-task-progress-reporting]]の再発）。
- **実データ再確認で判明**: 名古屋ページには7/22のN1①で駅別セクションが既にあり、久屋大通/名鉄名古屋/栄/大須/金山/本陣/栄生はカバー済み。**GSCで表示のある妙音通駅・内名駅は未カバー**だった
- **D2実施**: ①妙音通駅・内田橋・瑞穂(名古屋市南部)セクション新設=大手店舗が徒歩圏にないことを正直に書き、金山/大須への移動か出張買取を案内(捏造せず「確認できていない」と明記) ②title/descをメインクエリ(「金 買取 名古屋」81imp/33.7位・「名古屋 金買取」48imp/31.4位)向けに即答型へ=「名古屋の金買取おすすめ4社を比較｜名駅・栄・大須・駅別の店舗と今日の相場」
- **残P1(上田)**: 「上田駅 金 買取」53imp/**10.5位**=1ページ目目前のCTR改善。title「上田駅すぐの金買取はどこ？上田市のおすすめ店と今日の相場」+**ArticleSchema headlineも同期**(過去2回発生した「schema側だけ置換」の罠を回避)
- build EXIT0・方式B両push・本番確認・Indexing 2/2。効果1-2週GSC(名古屋の33.7位→1ページ目/上田のCTR転換)

### 2026-07-29 D3実行=インゴット規制クラスタ（MediaXAI「D3すすめよう」）✅本番反映済み
「日本 金インゴット 買取規制 2026」94imp/6.2位(着地=kin-nobebo-kaitori)の首位獲り。**ページに規制専用セクションが無いのに6位=正面回答で押し込める構図**だった。
- **一次情報調査(エージェント・全出典URL付き)の要旨**: ①2026年に個人の金売却への新規制は**存在しない**(令和8年度税制改正大綱=事業者の本人確認書類範囲追加のみ) ②変わること=支払調書様式変更(2026年8月)/免税リファンド方式(2026年11月)/犯収法施行規則改正=IC読み取り原則化(**2026年3月公布→2027年4月1日全面施行**)/税関の金密輸総合対策(2025年11月〜) ③変わらないこと=200万円超の支払調書・マイナンバー(2012年〜)/犯収法の現金200万円超本人確認/古物営業法/譲渡所得課税
- **実装**: nobeboに`<h2 id="kisei-2026">`規制セクション(結論ファースト+変わること表+変わらないことul+出典4リンク[財務省/国税庁/警察庁JAFIC/税関]+実務ポイント)+FAQ(可視+schema先頭)+title/desc/ArticleSchemaに「2026年の買取規制」+**クラスタ6ページ(zeikin/mynumber/tetsuzuki/souzoku/ingot-100g/kinka-ingot)から#kisei-2026へ逆リンクcallout**(`d3-kisei-202607`マーカー冪等)+nobeboから6ページへのchipリンク
- 副次修正: 「主な金インゴットのブランド」の**空table残骸**をLBMA公認5社の事実テーブルで充填(D1で見つけた同型残骸・純度ページ以外にも残存していた)/publisher URLがpages.devのまま→gold-biyori.comに修正/h1・最終更新6月→7月
- build EXIT0・空wrapper=0・方式B両push・本番確認・Indexing 7送信。効果1-2週GSC(6.2位→首位・クラスタページの浮上)

### 2026-07-28 do/know/go戦略v2 D1実行（MediaXAI「D1進めよう」）✅本番反映済み
戦略v2(`~/.openclaw/workspace/gold-strategy-do-know-go-2026-07-28.md`・7/28提出)のD1=純度クラスタ押し込み。診断でK22/K14はコンポーネント同等(P1で最適化済)だが**H2構成でK24に劣後**+**3ページに空table-wrapper残骸**(6/11誤値テーブル行削除の後遺症=見出しだけで中身なし)を発見。
- **K22追加**: ①K22vsK24vsK18純度別比較(WeightPriceTable複数purity=マトリクス表・比率0.917/1.222は実データ突合済) ②代表的な金貨・製品(クルーガーランド/イーグル/ソブリン=K22の事実テーブル) ③偽物・メッキ品の見分け方
- **K14追加**: ①K14vsK18vsK10純度別比較(0.778/1.400突合済) ②「14金の価格—購入時と買取の差の理由」(「14金 価格」54imp/43.5位の意図に回答・K24の田中貴金属節と同思想)。GP/メッキは既出のためスキップ
- **title/h1を検索量の多い表記先行に**: 「K22（22金）」→「22金（K22）」(22金買取144imp>k22 87imp)・K14同様
- **🐛空セクション修復**: K22×3/K14×2/K24×2の空table-wrapper→特徴テーブルは事実充填・重量目安/推移の空節は既存コンポーネント(WeightPriceTable/GoldPriceTrend)が担うため見出しごと削除。K24の金貨節にも事実テーブル充填
- **🐛K22のFAQ事実誤り修正**: 「メイプルリーフ金貨・ブリタニア金貨=K22」と記載→**メイプルリーフはK24**。クルーガーランド/イーグル/ソブリンに差替+K24金貨の注記追加。教訓: 金貨純度は貨幣ごとに異なる(22金=クルーガーランド/イーグル/ソブリン、24金=メイプル/ウィーン/カンガルー)
- build EXIT0・比率/タイトル/空wrapper=0をout検証・方式B両push・本番確認・Indexing k22=200/k14・k24=429(クォータ・翌日ローテで自動再送)。効果1-2週GSC(22金買取144imp/47位・k22買取価格87imp/41位・14金系126impの浮上)
...
### 2026-08-24 N3実行：駅名クエリの判別用に駅ページ2本（MediaXAI「n3進めて」）✅本番反映済み
- **背景**: GSC実測で駅名クエリ12個が pos7〜13 なのに**合計408表示・0クリック**。原因が「①6駅を1ページ（/articles/nagoya-kin-kaitori/）で兼ねていてタイトルに駅名が入らない」か「②駅名×買取はローカル枠に食われ通常検索がそもそも押されない」か判別できなかった。
- **対応**: **実店舗が公式で確認できた2駅だけ**ページ化して判別する。
  `/articles/sako-eki-kin-kaitori/`（栄生駅・買取大吉 中村栄生店）／`/articles/hisayaodori-eki-kin-kaitori/`（久屋大通駅・バイセル 名古屋栄セントラルパーク店）。
  ⚠️ **本陣・ささしまライブ・内名は作らない**（公式を当たっても最寄りとして案内している店舗が無い。店の無い駅までページ化するとtakushokuでスパム判定を受けた構造と同じになる）。
- 店舗情報は2026-08-24に公式店舗ページで再取得。⚠️ **買取大吉 名古屋錦店は公式の最寄り表記が「丸の内駅 徒歩4分」**で久屋大通からの分数は公式に無い → **勝手に計算せず「1駅」とだけ書く**（8/16と同じ判断を維持）。
- 中身の方針: 金の地金価値は駅で変わらないので、**駅ページで書けるのは立地の条件だけ**。栄生＝徒歩圏1軒で相見積もりが取りにくい／名駅まで1駅、久屋大通＝21時まで・地下街完結・完全個室、と具体で書き分けた。
- 名古屋ページの該当駅セクションから新ページへ導線2本。sitemap 111→113URL・再送信OK・Indexing 3/3。
- **判別方法（2〜3週後）**: ①該当クエリの着地URLが名古屋ページ→駅ページに変わるか ②クリックが1件でも出るか。**②が動かなければ仮説②（ローカル枠）が濃厚**なので、駅名クエリから撤退して別の切り口へ投資を移す。残り4駅を作るかはこの結果を見てから。
